...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for i in range(max_iterations):
            ...
            mis_class = mis_class * mod_y
            multiplier = np.dot(mis_class, mod_X)
...
def multiclass_train(X, y, C,
    ...
    mod_W = np.append(w, np.array([b]).T, axis=1)
    ...
    if gd_type == "sgd":
        ...
        for it in range(max_iterations):
            ...
            softmax =  np.divide(num,den)
            
            #logging.debug("gd softmax.value : " + str(softmax))
            ones = np.zeros(C)
            ones[y[n]] = 1
            softmax = softmax - ones
            softmax = softmax.reshape(C,1)
            ...
            sgd = np.dot(softmax, mod_X[n].reshape(1,D+1))
