...
def multiclass_train(X, y, C,
    ...
    if gd_type == "sgd":
        ...
        for it in range(max_iterations):
            ...
            W_xn = np.matmul(xn ,  mod_W.T)
            
            #logging.debug("sgd W_xn.shape : " + str(W_xn.shape))
            
            num = np.exp(np.subtract(W_xn , np.amax(W_xn)))
            ...
            #logging.debug("sgd softmax.shape : " + str(softmax[:,None].shape))
            logging.debug("sgd softmax[0] : " + str(softmax[0]))
            #logging.debug("sgd xn : " + str(xn))
        
            #logging.debug("sgd xn.value : " + str(xn))
            sgd = np.matmul(softmax[:,None], xn[None,:])
            #sgd = np.matmul(np.array([softmax]).T, np.array([xn]))
            #logging.debug("sgd sgd.shape : " + str(sgd.shape))
            logging.debug("sgd sgd.values[0] : " + str(sgd[0]))
        ...
        for _ in range(max_iterations):
            ...
            gd = np.matmul(softmax, mod_X)
            
            logging.debug("gd gd.shape : " + str(gd.shape))
            
            mod_W = mod_W - (step_size * gd)
