...
def multiclass_train(X, y, C,
    ...
    import logging
    logging.basicConfig(filename='weights.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
...
#            logging.debug("sgd numerator : " + str(numerator))
            
            denominator = np.sum(numerator)
#            logging.debug("sgd denominator : " + str(denominator))
            
            prob = [item / denominator for item in numerator]
            prob[y[n]] -= 1
            sgd_component = np.matmul(np.array([prob]).T, np.array([updated_X[n]]))
#            logging.debug("sgd sgd_component : " + str(sgd_component))
            updated_w = updated_w - (step_size) * sgd_component
    
#        logging.debug("sgd weights : " +  str(updated_w))
...
#        logging.debug("gd weights : " + str(updated_w))
    
    else:
        raise "Undefined algorithm."

    logging.debug("mod_W : " + str(updated_w))
    b = updated_w[:, -1]
